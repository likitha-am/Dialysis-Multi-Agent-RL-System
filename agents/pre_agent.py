...
class PreDialysisAgent(nn.Module):
    def __init__(self):
        super(PreDialysisAgent, self).__init__()

        # Encoders
        self.num_encoder = NumericEncoder(input_dim=7, output_dim=32)
        self.cat_encoder = CategoricalEncoder(num_categories=10, embed_dim=16)
        self.img_encoder = ImageEncoder(output_dim=64)
        self.txt_encoder = TextEncoder(vocab_size=1000, embed_dim=32, hidden_dim=64)

        # Fusion
        fusion_input_dim = 32 + (16 * 3) + 64 + 64
        ...
        # Agent Core
        self.core = AgentCore(action_dim=5)
